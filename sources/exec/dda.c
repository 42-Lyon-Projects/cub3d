...
/*   Updated: 2024/04/23 15:50:31 by lunagda          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sidestep_dist(t_cub3d *data, t_ray *ray, t_dda *dda)
{
	...
	{
		dda->step_x = -1;
		dda->side_dist.x = (data->map.player.x_pos - dda->map_x) * dda->delta_dist.x;
	}
	else
	{
		dda->step_x = 1;
		dda->side_dist.x = (dda->map_x + 1.0 - data->map.player.x_pos) * dda->delta_dist.x;
	}
	if (ray->ray_dir.y < 0)
	{
		dda->step_y = -1;
		dda->side_dist.y = (data->map.player.y_pos - dda->map_y) * dda->delta_dist.y;
	}
	else
	{
		dda->step_y = 1;
		dda->side_dist.y = (dda->map_y + 1.0 - data->map.player.y_pos) * dda->delta_dist.y;
	}
}
