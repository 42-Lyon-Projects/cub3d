...
/*   Updated: 2024/05/07 13:21:15 by lunagda          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_cub3d *data, t_dda dda, t_ray ray, int x)
{
	...
	while (y < dda.draw_end + 1)
	{
		if (dda.line_height == 0)
			break ;
		tex_x = (int)(data->wall_x * data->wall.width);
		tex_y = (int)(((y - (data->res_y / 2) + (dda.line_height / 2))
					* data->wall.height) / dda.line_height);
...
void	raycast_helper(t_cub3d *data, t_ray *ray, t_dda *dda, int x)
{
	...
	ray->ray_dir.x = data->map->player.x_dir + data->map->player.plane_x
		* ray->camera_x;
	ray->ray_dir.y = data->map->player.y_dir + data->map->player.plane_y
		* ray->camera_x;
